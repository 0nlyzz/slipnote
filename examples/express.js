...
// This is not a production server. Add authentication before exposing
// private note contents over a network.
...
app.post('/slipnotes', asyncRoute(async (req, res) => {
...
app.get('/slipnotes/pull', asyncRoute(async (req, res) => {
...
app.get('/slipnotes/receipts', asyncRoute(async (req, res) => {
...
app.post('/slipnotes/:id/reply', asyncRoute(async (req, res) => {
...
app.get('/slipnotes/replies', asyncRoute(async (req, res) => {
  const newReplies = await notes.pullReplies();
  res.json(newReplies);
}));

app.use((err, req, res, _next) => {
  const isInputError = typeof err.message === 'string' && err.message.startsWith('slipnote:');
  res.status(isInputError ? 400 : 500).json({
    error: isInputError ? err.message : 'internal server error',
  });
});
